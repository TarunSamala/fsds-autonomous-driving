...
class SlamV2Node(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('observation_threshold', 3)
        ...
        # Lap detection with hysteresis
        self.lap_count = 0
        self.last_lap_start_x = None
        self.last_lap_start_y = None
    ...
    def odom_callback(self, msg: Odometry):
        """Update robot pose and detect lap completion"""
        pose = msg.pose.pose
        ...
        # Improved lap detection with hysteresis (only state-based)
        if self.lap_state == "SEARCHING":
            if self.last_lap_start_x is None:
                self.last_lap_start_x = car_x
                self.last_lap_start_y = car_y

            # Measure distance traveled from start
            dist_traveled = abs(car_x - self.last_lap_start_x) + abs(car_y - self.last_lap_start_y)
            if dist_traveled > 10.0:  # Must travel 10m before entering lap state
                self.lap_state = "IN_LAP"

        elif self.lap_state == "IN_LAP":
            # Check if returned to start
            dist_to_start = np.sqrt(
                (car_x - self.last_lap_start_x)**2 + 
                (car_y - self.last_lap_start_y)**2
            )
            if dist_to_start < 1.5:  # Within 1.5m of start
                self.lap_count += 1
                self.get_logger().info(f"🔄 Lap {self.lap_count} completed!")
                self.lap_state = "SEARCHING"  # Reset to search state
                self.last_lap_start_x = car_x
                self.last_lap_start_y = car_y
